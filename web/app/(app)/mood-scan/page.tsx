    ...
    <div className="max-w-4xl mx-auto space-y-10 pb-24 stagger-children">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-tighter flex items-center gap-3">
            ...
            Mood <span className="text-indigo-500">Scan</span>
          </h1>
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-indigo-600/10 border border-indigo-500/20 rounded-full">
          ...
          <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">AI-Powered</span>
        </div>
      </div>
      ...
      {!result && (
        <div className="relative group">
          ...
          <Card className="relative bg-slate-900 border-white/5 rounded-[32px] overflow-hidden">
            <CardContent className="p-8 space-y-6">
              {/* URL Input */}
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-3">
                  <Link2 className="w-4 h-4 text-indigo-400" />
                  <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Paste a URL or describe content</span>
                </div>
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    <Input
                      ...
                      className="h-14 text-base pl-4 pr-4 bg-slate-800/50 border-white/5 rounded-2xl focus:border-indigo-500/50"
                      disabled={loading}
                    />
                  </div>
                  <Button
                    ...
                    className="h-14 px-8 bg-indigo-600 hover:bg-indigo-500 rounded-2xl font-black text-sm shadow-lg shadow-indigo-600/20"
                  >
                    {loading ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Quick Platform Scan</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {PLATFORM_PRESETS.map(p => (
                    <button
                      ...
                      disabled={loading}
                      className="group/btn flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800/30 border border-white/5 hover:border-indigo-500/20 transition-all text-sm disabled:opacity-50"
                    >
                      <span style={{ color: p.color }} className="group-hover/btn:scale-110 transition-transform">
                        {p.icon}
                      </span>
                      <span className="text-slate-400 font-bold group-hover/btn:text-white transition-colors text-xs">
                        {p.label}
                      </span>
                    </button>
                  ))}
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16 space-y-6 animate-fade-in-up">
          ...
          <div className="text-center space-y-2">
            <p className="text-sm font-bold text-indigo-400">{scanPhase}</p>
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Analyzing 5 Mood Dimensions</p>
          </div>
      ...
      {error && (
        <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-center animate-fade-in-up">
          ...
          <button
            ...
            className="mt-3 text-xs text-slate-500 hover:text-indigo-400 font-bold transition-colors"
          >
            Try Again
          </button>
        </div>
      ...
      {/* Scan History */}
      {!loading && (
        <Card className="bg-slate-900/30 border-white/5 rounded-[32px]">
