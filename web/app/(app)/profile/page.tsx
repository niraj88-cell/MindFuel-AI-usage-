...
function StatCard({ icon: Icon, color, bg, value, label }: {
  ...
  return (
    <div className={`flex flex-col items-center justify-center p-6 rounded-3xl ${bg} border border-white/5 text-center`}>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-20 stagger-children">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Your account, stats, and data settings.</p>
      </div>

      {/* Identity Card */}
      <div className="relative">
        <div className="absolute -inset-px bg-gradient-to-r from-indigo-500/20 to-purple-500/20 rounded-[28px] blur-sm" />
        <div className="relative bg-slate-900/80 backdrop-blur-xl border border-white/5 rounded-[28px] p-6">
          <div className="flex items-center gap-5">
            {/* Avatar */}
            <div className="relative">
              <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-indigo-500/30 to-purple-500/30 border border-indigo-500/30 flex items-center justify-center text-3xl font-black text-indigo-300">
                {initials}
              </div>
              {isPremium && (
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatCard icon={Brain} color="text-indigo-400" bg="bg-indigo-500/5" value={data?.totalLogs || 0} label="Logs" />
        <StatCard icon={Flame} color="text-rose-400" bg="bg-rose-500/5" value={data?.highestStreak || 0} label="Best Streak" />
        <StatCard icon={BarChart3} color="text-emerald-400" bg="bg-emerald-500/5" value={`${data?.avgScore || 0}/100`} label="Avg Score" />
        <StatCard icon={Award} color="text-amber-400" bg="bg-amber-500/5" value={data?.totalMoodEntries || 0} label="Mood Checks" />
      </div>
      ...
      <div className="bg-slate-900/50 border border-white/5 rounded-3xl p-6 space-y-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Download your last 90 days of data (5 exports/day)</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center gap-2.5 p-4 rounded-2xl bg-slate-800/50 border border-white/5 hover:border-indigo-500/30 hover:bg-indigo-500/5 transition-all cursor-pointer disabled:opacity-50 group"
          >
            {exporting === 'json'
            ...
            <div className="text-left">
              ...
              <div className="text-[10px] text-slate-500">Full data</div>
            </div>
          </button>
          ...
          <button
            ...
            disabled={exporting !== null}
            className="flex items-center gap-2.5 p-4 rounded-2xl bg-slate-800/50 border border-white/5 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all cursor-pointer disabled:opacity-50 group"
          >
            {exporting === 'csv'
            ...
            <div className="text-left">
              ...
              <div className="text-[10px] text-slate-500">Spreadsheet</div>
            </div>
          </button>
        </div>
      </div>
      ...
      {/* Security */}
      <div className="bg-slate-900/50 border border-white/5 rounded-3xl p-6 space-y-1">
        <div className="flex items-center gap-3 mb-4">
          <Shield className="w-5 h-5 text-indigo-400" />
          <h3 className="text-sm font-black text-white">Security</h3>
        </div>

        <Link href="/forgot-password" className="flex items-center justify-between p-4 rounded-2xl bg-slate-800/30 border border-white/5 hover:border-white/10 hover:bg-slate-800/60 transition-all group">
        ...
        <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-800/30 border border-white/5 mt-1">
          <div className="flex items-center gap-3">
            <Shield className="w-4 h-4 text-emerald-400" />
            <div>
              <div className="text-sm font-bold text-white">Data Encryption</div>
              <div className="text-xs text-slate-500">All data encrypted at rest via Supabase</div>
            </div>
          </div>
          <span className="text-[10px] font-black text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded-full">ACTIVE</span>
        </div>
      </div>
      ...
      {!isPremium && (
        <div className="relative overflow-hidden rounded-3xl">
          ...
          <div className="relative p-6 flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-1">Unlock Premium</p>
              <p className="text-sm font-bold text-white">Unlimited logs, 500 coach messages/hr, priority AI</p>
            </div>
            <Link href="/subscription">
              <Button size="sm" className="shrink-0 bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-600/30">
                Upgrade
              </Button>
            </Link>
          </div>
